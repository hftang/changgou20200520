...
public class OrderServiceImpl implements OrderService {
    ...
    public Order add(Order order) {
        ...
        //查出所有购物车
        List<OrderItem> orderItems = this.cartService.list(order.getUsername());
        int totalMoney = 0;
        int totalPayMoney = 0;
        int num = 0;
        ...
        //添加订单明细
        for (OrderItem orderItem : orderItems) {
            orderItem.setId("No." + this.idWorker.nextId());
        ...
        //减少库存
        this.skuFeign.decrCount(order.getUsername());

        //添加用户积分
        userFeign.addUserPoints(10);


        //如果是线上支付的话 就把订单号存入redis

        if ("1".equalsIgnoreCase(order.getPayType())) {
            this.redisTemplate.boundHashOps("Order").put(order.getId(), order);
        }

        //清除redis中的缓存
//        this.redisTemplate.delete("cart_" + order.getUsername());
    ...
    public void deleteOrder(String id) {
        Order order = (Order) this.redisTemplate.boundHashOps("Order").get(id);
        ...
        //删除缓存
        this.redisTemplate.boundHashOps("order").delete(id);
    ...
    public void updateOrderStatus(String orderId, String transactionId) {
        ...
        if(order!=null){
            ...
            //删除 redis中
            this.redisTemplate.boundHashOps("Order").delete(orderId);
        }
